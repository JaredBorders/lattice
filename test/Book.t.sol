...
contract Bookest is Test {
    ...
    function setUp() public {
        ...
        book =
            new Book(address(clearinghouse), address(numeraire), address(index));
        ...
        // Set up price levels
        price1 = Price.wrap(1000 * 10 ** 18); // 1000 USD per ETH
        price2 = Price.wrap(1100 * 10 ** 18); // 1100 USD per ETH

        vm.prank(address(book));
        numeraire.approve(address(clearinghouse), type(uint256).max);
        vm.prank(address(book));
        index.approve(address(clearinghouse), type(uint256).max);
    }
    ...
    function test_place_ask() public {
        ...
        assertEq(index.balanceOf(address(book)), initialBookBalance + 1 ether);
    }
